...
<div class="col-md-10">
	...
	<form id="sendfrm" action='<c:url value="/class/classBoardEdit.do"/>' enctype="multipart/form-data" method="post">
		<br>
		<input type="hidden" value="${vo.cNo }" name="cNo" id ="cNo">
		<input type="hidden" value="${vo.cbNo }" name="cbNo" id ="cbNo">
		<div class="container">
			<div class="row">
		...
		<div class="section" id="section2">
				<div class="intro container">
						<p>
						<label for="image">등록이미지1:
						<c:if test="${!empty fileList[0] }">
							${fileList[0].fOriginalfilename}
						</c:if>
						<c:if test="${empty fileList[0] }">
							등록된 이미지 없습니다.
						</c:if>
					...
					<p>
						<label for="image">등록이미지2:</label>
						<c:if test="${!empty fileList[1] }">
							${fileList[1].fOriginalfilename}
						</c:if>
						<c:if test="${empty fileList[1] }">
							등록된 이미지 없습니다.
						</c:if>
					...
					<p>
						<label for="image">등록이미지3:</label> 
						<c:if test="${!empty fileList[2] }">
							${fileList[2].fOriginalfilename}
						</c:if>
						<c:if test="${empty fileList[2] }">
							등록된 이미지 없습니다.
						</c:if>
	...
	<div class="col-md-10">
<textarea class="ckeditor" id="cbContent" name="cbContent" style="margin-top: 400px">
${vo.cbContent }
